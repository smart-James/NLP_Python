...
@torch.no_grad()  # 关闭梯度计算，因为这里我们只是估计，而不是更新参数
def estimate_loss():
    out = {}  # 定义一个字典，用于存储估计的损失
    model.eval()  # 将模型设置为评估模式
    for split in ['train', 'val']:  # 遍历训练集和验证集
        losses = torch.zeros(eval_iters)  # 定义一个张量，用于存储每次迭代的损失
        for k in range(eval_iters):  # 遍历每次迭代
            X, Y = get_batch(split)  # 获取训练集或验证集的批次数据
            logits, loss = model(X, Y)  # 将数据输入模型，获取输出和损失
            losses[k] = loss.item()  # 将损失存储在张量中
            
        out[split] = losses.mean()  # 将训练集和验证集的损失平均值存储在字典中
        
    model.train()  # 将模型设置为训练模式
    return out  # 返回字典，包含训练集和验证集的估计损失
...
# 定义优化器
# AdamW 是一种改进的 Adam 算法，它在更新权重时会考虑权重衰减（L2正则化）的作用，从而更好地控制模型复杂度。

optimizer = torch.optim.AdamW(model.parameters(), lr=learning_rate)


for iter in range(max_iters):
    if (iter+1) % eval_iters == 0:
...
# 这段代码的主要功能是将一个训练好的模型对象（model）序列化并保存到一个二进制文件（model-01.pkl）中。
# 通过这种方式，可以在后续的代码中加载这个模型，而无需重新训练。这对于模型的持久化和共享非常有用。
